...
class Performance(Vendor):
	...
	special_cases = {
		...
		'72977': {'unit': 'LB', 'pack': 5},
		'T9027': {'unit': 'LB', 'pack': 5},
		'LD458': {'unit': 'LB', 'pack': 15},  # Bacon
		'50778': {'unit': 'EA', 'pack': 106}, # Sausage Patties
		'FE128': {'unit': 'EA', 'pack': 160}, # Vegan Sausage Patties
		'N0058': {'unit': 'EA', 'pack': 168}, # Egg Patties
		'J2220': {'unit': 'EA', 'pack': 54},  # Croissant - Butter
		'CW526': {'unit': 'EA', 'pack': 66},  # Croissant - Chocolate
		'HH264': {'unit': 'OZ', 'pack': 160}, # Cheese - Muenster
	}
...
class Renzi(Vendor):
	...
	special_cases = {
		...
		'37019': {'unit': 'EA', 'pack': 106}, # Sausage Patties
		'11458': {'unit': 'EA', 'pack': 45},  # Vegan Sausage Patties
		'27098': {'unit': 'EA', 'pack': 168}, # Egg Patties
	}
...
class Sysco(Vendor):
	...
	special_cases = {
		'7421761': {'unit': 'EA', 'pack': 106}, # Sausage Patties
		'7143223': {'unit': 'EA', 'pack': 96},  # Vegan Sausage Patties
		'4617771': {'unit': 'EA', 'pack': 168}, # Egg Patties
		'2700171': {'unit': 'EA', 'pack': 54},  # Croissant - Butter
		'2700817': {'unit': 'EA', 'pack': 66},  # Croissant - Almond
		'2700827': {'unit': 'EA', 'pack': 66},  # Croissant - Chocolate
		'1865161': {'unit': 'OZ', 'pack': 384}, # Half and Half
		'6472502': {'unit': 'OZ', 'pack': 320}, # Yogurt - BIB
		'0017354': {'unit': 'EA', 'pack': 120}, # Hash Browns
	}
	...
	def get_info(self):
		...
		with open(self.path_to_pricing_sheet) as sysco_info:
			...
			for pos, row in enumerate(reader):
				...
				if item_sku in self.special_cases:
					quantity = self.special_cases[item_sku]['pack']
					units = self.special_cases[item_sku]['unit']
				else:
...
def price_compare(path_to_pricing_sheet: str) -> None:
	...
	for pos, row in enumerate(sheet.iter_rows()):
		...
		if pos == 0:
			excluded_values = ['Item', 'Cheapest', 'Preferred']
			for col in row:
		...
		for i in range(4, 4*(len(vendors) + 1), 4):
...
if __name__ == '__main__':
	...
	'''NEED TO FIX UNITS ON A MAJORITY OF ITEMS!!!'''
	base_path = Path(__file__).parent.resolve()

	pricing_sheet_path		 = join(base_path, 'GeneralItemPricing.xlsx')
