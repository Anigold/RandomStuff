...
class SqlRepository(Generic[RecordT, DomainT]):
    """Small generic repository for shared SQLAlchemy CRUD mechanics."""
    ...
    def add(self, domain_obj: DomainT) -> None:
    ...
    def _list_by_statement(self, statement) -> list[DomainT]:
        records: Sequence[RecordT] = self._session.scalars(statement).all()
        return [self._to_domain(record) for record in records]

    def _one_or_none_by_statement(self, statement) -> DomainT | None:
