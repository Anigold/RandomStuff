...
from .paths import CLI_HISTORY_FILE
...
class CLI:
    ...
    def __init__(self) -> None:
        ...
        self._register_builtin_commands(self.command_context)
        self._build_ui()
    ...
    def _build_ui(self) -> None:
        ...
        self.command_completer = CLICompleter(self)

        self.input_field = TextArea(
            height=1,
            prompt=lambda: self._input_prompt,
            password=Condition(
                lambda: self._secret_input_handler is not None
            ),
            multiline=False,
            wrap_lines=False,
    ...
    def _build_keybindings(self):
        ...
        def _(event) -> None:
            ...
            if self._secret_input_handler is not None:
                secret = self.input_field.text

                self.input_field.buffer.reset(
                    append_to_history=False,
                )

                self._submit_secret_input(secret)
                return
    ...
    def _register_builtin_commands(self, context: CommandContext | None = None) -> None:
        context = context or self.command_context
        self.load_commands_from_package("apps.cli.commands.builtin", context)

    def load_commands_from_package(self, package_name: str, context: CommandContext) -> None:
    ...
    def _begin_secret_input(
        ...
        if self._secret_input_handler is not None:
            self._handle_command_result(
                CommandResult.error(
                    "Another secure input request is already active."
                )
            )
            return

        self._secret_input_handler = request.handler
        self._secret_cancel_message = request.cancel_message

        self._input_prompt = request.prompt

        # Disable autocomplete while entering secrets.
    ...
    def _submit_secret_input(
        ...
        handler = self._secret_input_handler
    ...
    def _reset_secret_input(self) -> None:
        ...
        self.input_field.completer = self.command_completer
    ...
    def _dispatch_command(self, command: str, args: list[str]) -> None:
        ...
        try:
            ...
            # self.logger.warning(
            #     "Command '%s' argument parsing failed with exit code %s.",
            #     command,
            #     exc.code,
            # )
