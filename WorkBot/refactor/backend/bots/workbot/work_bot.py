...
class WorkBot:
    ...
    def get_order_file_path(self, order: Order, fmt: str = 'pdf') -> Path:
        return self.order_coordinator.get_order_file_path(order, format=fmt)
    ...
    def input_transfers(self):
        transfer_file_dir = self.transfer_coordinator.get_transfer_files_directory()
        transfers = [
            TransferCoordinator.create_transfer_from_excel(file)
            for file in transfer_file_dir.iterdir() if file.is_file()
        ]
        self.craft_bot.input_transfers(transfers)
    ...
    def generate_vendor_upload_files(
        ...
        order_file_paths = self.order_coordinator.get_orders_from_file(stores=stores, vendors=vendors)
        self.logger.info(f"Found {len(order_file_paths)} order files to process.")
