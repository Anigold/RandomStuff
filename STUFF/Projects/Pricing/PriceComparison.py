	...
	def update_skus_to_file(self) -> None:
		...
		for item in prices:
			if item[0] in skus:
				...
				sheet.cell(row=skus[item[0]], column=self.skus_col+2).value = cost_per_unit
				sheet.cell(row=skus[item[0]], column=self.skus_col+3).value = cost
...
class Palmer(Vendor):
	...
	def get_info(self):
		item_info = []
		with open(self.path_to_pricing_sheet) as palmer_info:
			reader = csv.reader(palmer_info, delimiter=',')
			for pos, row in enumerate(reader):
				...
				item_sku 		= row[0]
				item_name 		= row[2]
				quantity, units = Vendor.helper_format_size_units(1, row[4])
				cost 			= row[5]
...
class Hoffmire(Vendor):
	...
	def get_units_from_pack_size(pack_size: str) -> str:
		...
		if 'x' not in pack_size:
			for char in pack_size:
				if char.isnumeric or char in quantity_symbols: quantity.append(char)
				else: units.append(char)
			return [quantity, units]

		temp = pack_size.replace(' ', '').split['x']
		
	def get_info(self):
		item_info = []
		with open(self.path_to_pricing_sheet) as hoffmire_info:
			reader = csv.reader(hoffmire_info, delimiter=',')
			for pos, row in enumerate(reader):
...
def price_compare(path_to_pricing_sheet: str) -> None:
	...
	for pos, row in enumerate(sheet.iter_rows()):
		...
		for i in range(3, 3*(len(vendors)+1), 3):
		...
		if prices: 
			...
			for index in indices:
				if sheet.cell(row=pos, column=2).value:
					sheet.cell(row=pos, column=2).value = f'{sheet.cell(row=pos, column=2).value} {vendors[index]} '
				else:
